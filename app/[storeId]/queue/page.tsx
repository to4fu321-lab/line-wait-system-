...
const SCHOOLS = [
  '○○高等学校', '○○高等学校（2年）', '△△中学校', '△△中学校（新入生）',
  '□□高等学校', '◇◇中学校', '◎◎高等学校', 'その他（直接入力）',
]
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 gap-6"
      ...
      <div className="text-center text-white">
        ...
        <p className="text-white/80 text-base">順番が来たらLINEで通知が届きます</p>
      </div>
      <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl space-y-4">
        <a href={addUrl}
          className="w-full bg-[#06C755] text-white text-xl font-black py-5 rounded-2xl flex items-center justify-center gap-3 active:scale-95 transition-transform shadow-lg">
          <MessageCircle size={24} />① 友達追加する
        </a>
        <button onClick={handleCheck} disabled={checking}
          className="w-full text-white text-lg font-bold py-4 rounded-2xl disabled:opacity-60 flex items-center justify-center gap-2 active:scale-95 transition-transform"
          style={{ background: `linear-gradient(135deg, ${theme.colors.primary}, ${theme.colors.primaryDark})` }}>
          {checking ? <><Loader2 size={18} className="animate-spin" />確認中...</> : '② 追加済み → 受付へ進む'}
        </button>
        {failed && <p className="text-red-500 text-sm text-center">友達追加が確認できません。</p>}
      </div>
    </div>
  )
}
...
function RegisterView({ storeId, storeName, onComplete, lineProfile, inLineApp, allowRemote, existingCustomers = [] }: {
  ...
  const handleCustomerNameChange = (val: string) => {
    ...
    if (!kanaEditedRef.current && !isComposingRef.current && !kanaBlockRef.current) {
      setCustomerKana(toKatakana(val))
    }
    kanaBlockRef.current = false
    setError(null)
  }
  ...
  const handleSelectCustomer = (c: Customer) => {
    ...
    if (c.school_name) {
      if (SCHOOLS.includes(c.school_name)) { setSchoolName(c.school_name); setShowCustomInput(false) }
      else { setShowCustomInput(true); setCustomSchool(c.school_name) }
    }
    setError(null)
  }

  useEffect(() => {
    if (!nameEditedRef.current && lineProfile?.displayName) setCustomerName(lineProfile.displayName)
  ...
  const accentBorder = (active: boolean) =>
    active ? `border-[color:var(--theme-primary)] bg-white` : 'border-gray-100 bg-gray-50'

  return (
    <div className="min-h-screen flex flex-col" style={{ background: `linear-gradient(135deg, ${theme.colors.primary} 0%, ${theme.colors.primaryDark} 100%)` }}>
      <div className="px-5 pt-6 pb-4 text-white">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-black tracking-tight">順番待ち受付</h1>
            {storeName && <p className="text-white/60 text-xs mt-0.5">{storeName}</p>}
          </div>
          {waitingCount !== null && (
            <div className="text-right bg-white/15 rounded-2xl px-4 py-2 backdrop-blur-sm border border-white/20">
              <p className="text-white/60 text-xs font-bold">現在の待ち</p>
              <p className="text-2xl font-black leading-none text-white">{waitingCount}<span className="text-xs font-bold ml-0.5">組</span></p>
            </div>
          )}
        </div>
        {lineProfile ? (
          <div className="inline-flex items-center gap-1.5 bg-white/10 border border-white/20 rounded-full px-2.5 py-1 mt-2 text-xs text-white">
            <MessageCircle size={10} />LINE連携済み
          </div>
        ) : !inLineApp ? (
          <div className="inline-flex items-center gap-1.5 bg-yellow-400/20 border border-yellow-300/30 rounded-full px-2.5 py-1 mt-2 text-xs text-white">
            ⚠️ LINEで開くと通知が届きます
          </div>
        ) : null}
      </div>
      ...
      <div className="flex-1 bg-white rounded-t-3xl px-4 pt-4 pb-6 animate-slide-up shadow-2xl overflow-y-auto">
        <div className="max-w-md mx-auto space-y-3">
          ...
          {existingCustomers.length > 0 && (
            <div className="rounded-xl p-3" style={{ background: `rgb(${theme.colors.primaryRgb} / 0.07)`, border: `1px solid rgb(${theme.colors.primaryRgb} / 0.15)` }}>
              <p className="text-xs font-bold mb-2" style={{ color: theme.colors.primary }}>👋 登録済みのお子様から選択</p>
              <div className="flex flex-wrap gap-2">
                {existingCustomers.map(c => (
                  <button key={c.id} type="button" onClick={() => handleSelectCustomer(c)}
                    className={`text-xs font-bold px-3 py-1.5 rounded-full active:scale-95 transition-all border-2 ${selectedCustomerId === c.id ? 'text-white' : 'bg-white/70'}`}
                    style={selectedCustomerId === c.id
                      ? { background: theme.colors.primary, borderColor: theme.colors.primary }
                      : { color: theme.colors.primary, borderColor: `rgb(${theme.colors.primaryRgb} / 0.3)` }}>
                    {c.name}
                  </button>
                ))}
              </div>
              <p className="text-xs mt-1.5 text-zinc-400">タップすると情報が入力されます</p>
            </div>
          )}

          <FormField label="学校名" required>
            <div className="relative">
              <select className="w-full appearance-none text-sm border-2 border-gray-100 bg-gray-50 rounded-xl px-3 py-2.5 pr-9 focus:outline-none text-gray-800 transition-all"
            ...
            {showCustomInput && (
              <input type="text" className="mt-1.5 w-full text-sm border-2 border-gray-200 bg-gray-50 rounded-xl px-3 py-2.5 focus:outline-none transition-all"
                placeholder="学校名を入力してください" value={customSchool}
                onChange={e => { setCustomSchool(e.target.value); setError(null) }} autoFocus />
            )}
          </FormField>
          ...
          <div className="grid grid-cols-2 gap-3">
            <FormField label="氏名（保護者様）" required>
              <input type="text" inputMode="text" autoComplete="name"
                className="w-full text-sm border-2 border-gray-100 bg-gray-50 rounded-xl px-3 py-2.5 focus:outline-none transition-all"
                placeholder="例：山田 太郎" value={customerName}
            ...
            <FormField label="お子様のお名前" required>
              <input type="text" inputMode="text" autoComplete="off"
                className="w-full text-sm border-2 border-gray-100 bg-gray-50 rounded-xl px-3 py-2.5 focus:outline-none transition-all"
                placeholder="例：山田 花子" value={childName}
          ...
          <FormField label="フリガナ（保護者様）">
            <input type="text" inputMode="text"
              className="w-full text-sm border-2 border-gray-100 bg-gray-50 rounded-xl px-3 py-2.5 focus:outline-none transition-all"
              placeholder="ヤマダ タロウ" value={customerKana}
              ...
              <div className="grid grid-cols-3 gap-1">
                {GENDER_OPTIONS.map(opt => (
                  <button key={opt.value} type="button" onClick={() => setGender(opt.value)}
                    className={`py-2 rounded-xl border-2 text-center transition-all active:scale-95 ${accentBorder(gender === opt.value)}`}>
                    <div className="text-lg">{opt.icon}</div>
                    <div className={`text-xs font-bold mt-0.5 ${gender === opt.value ? 'text-[color:var(--theme-primary)]' : 'text-gray-500'}`}>{opt.label}</div>
                  </button>
                ))}
              </div>
            </FormField>
              ...
              <div className="grid grid-cols-3 gap-1">
                {(['fitting', 'pickup', 'other'] as QueueCategory[]).map(cat => (
                  <button key={cat} type="button" onClick={() => setCategory(cat)}
                    className={`py-2 rounded-xl border-2 text-center transition-all active:scale-95 ${accentBorder(category === cat)}`}>
                    <div className="text-lg">{CATEGORY_ICONS[cat]}</div>
                    <div className={`text-xs font-bold mt-0.5 ${category === cat ? 'text-[color:var(--theme-primary)]' : 'text-gray-500'}`}>{CATEGORY_LABELS[cat]}</div>
                  </button>
                ))}
              </div>
            </FormField>
          ...
          {allowRemote && (
            <button type="button" onClick={() => setIsRemote(v => !v)}
              className={`w-full py-2.5 rounded-xl border-2 px-4 flex items-center justify-between transition-all active:scale-[0.98] ${isRemote ? 'bg-white' : 'border-gray-100 bg-gray-50'}`}
              style={isRemote ? { borderColor: theme.colors.primary } : {}}>
              <div className="text-left">
                <p className={`font-bold text-sm ${isRemote ? 'text-[color:var(--theme-primary)]' : 'text-gray-600'}`}>🏠 遠隔チェックイン</p>
                <p className="text-xs text-gray-400">今すぐ来店しない</p>
              </div>
              <div className="w-10 h-5 rounded-full transition-colors shrink-0" style={{ background: isRemote ? theme.colors.primary : '#d1d5db' }}>
          ...
          <button type="button" onClick={handleSubmit} disabled={loading}
            className="w-full text-white text-lg font-black py-4 rounded-2xl shadow-lg active:scale-95 transition-all disabled:opacity-60 flex items-center justify-center gap-2"
...
function DetailsView({ ticket, storeId, onComplete, onSkip }: {
  ...
  useEffect(() => {
    supabase.from('queues')
      ...
      .lt('ticket_number', ticket.ticket_number).gte('created_at', getTodayStart())
        ...
        <div className="max-w-md mx-auto space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <FormField label="お子様の身長（cm）">
              <input type="number" inputMode="numeric" className="w-full text-base border-2 border-gray-100 bg-gray-50 rounded-xl px-3 py-3 focus:border-emerald-400 focus:outline-none transition-all"
                placeholder="例：155" value={height} onChange={e => setHeight(e.target.value)} />
            </FormField>
            <FormField label="お子様の体重（kg）">
              <input type="number" inputMode="numeric" className="w-full text-base border-2 border-gray-100 bg-gray-50 rounded-xl px-3 py-3 focus:border-emerald-400 focus:outline-none transition-all"
                placeholder="例：50" value={weight} onChange={e => setWeight(e.target.value)} />
            </FormField>
          </div>
          <FormField label="保護者の電話番号">
            <input type="tel" inputMode="tel" autoComplete="tel" className="w-full text-base border-2 border-gray-100 bg-gray-50 rounded-xl px-3 py-3 focus:border-emerald-400 focus:outline-none transition-all"
              placeholder="例：090-1234-5678" value={parentPhone} onChange={e => setParentPhone(e.target.value)} />
          </FormField>
          <button type="button" onClick={handleSubmit} disabled={loading}
...
function WaitingView({ ticket, waitingAhead, waitThresholds, onStatusChange, onCheckIn, onCancel, storeId, storeName }: {
  ...
  const handleCheckin = async () => {
    ...
    setCheckinLoading(false); onCheckIn()
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-violet-900 to-blue-900 flex flex-col">
      ...
      <div className="relative px-6 py-6 text-center text-white">
        ...
        <h1 className="text-2xl font-black tracking-tight">{isRemoteWaiting ? '🏠 遠隔待ち受付中' : '受付完了'}</h1>
      ...
      <main className="relative flex-1 flex flex-col items-center px-5 pb-10 max-w-md mx-auto w-full">
        <div className="w-full bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl p-8 text-center animate-slide-up">
          ...
          <div className="mt-6 bg-white/10 backdrop-blur-sm border border-white/15 rounded-2xl p-5">
            ...
            {waitMsg && !isRemoteWaiting && <p className="text-white/80 text-sm font-medium mt-3 leading-relaxed">{waitMsg}</p>}
            {isRemoteWaiting && <p className="text-indigo-200/70 text-sm mt-3">店舗に到着したらチェックインしてください</p>}
          </div>
          ...
          {ticket.line_user_id ? (
            <div className="mt-4 inline-flex items-center gap-2 bg-emerald-400/20 border border-emerald-400/30 rounded-full px-4 py-2 text-emerald-300">
              <MessageCircle size={13} /><span className="text-sm font-medium">LINEで呼出通知が届きます</span>
            </div>
          ) : (
        ...
        {isRemoteWaiting && (
          <div className="w-full mt-4 space-y-3 animate-fade-in">
            <button onClick={handleCheckin} disabled={checkinLoading}
              className="w-full bg-gradient-to-r from-emerald-500 to-green-500 text-white text-xl font-black py-6 rounded-2xl shadow-2xl shadow-emerald-900/50 active:scale-95 transition-all disabled:opacity-60 flex items-center justify-center gap-3">
              {checkinLoading ? <><Loader2 size={22} className="animate-spin" />チェックイン中...</> : <><MapPin size={22} />店舗付近に到着しました</>}
            </button>
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-end justify-center z-50">
          <div className="bg-zinc-900 border border-white/10 rounded-t-3xl p-6 w-full max-w-md animate-fade-in">
            ...
            <p className="text-zinc-400 text-sm text-center mb-6 leading-relaxed">
              再度並ぶ場合は最後尾からとなります。この操作は取り消せません。
            </p>
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => setShowCancelModal(false)}
                className="py-4 rounded-xl bg-zinc-800 text-white font-bold active:scale-95 transition-transform">戻る</button>
              <button onClick={async () => { setCancellingTicket(true); await onCancel() }} disabled={cancellingTicket}
                className="py-4 rounded-xl bg-red-600 text-white font-black active:scale-95 transition-all disabled:opacity-60 flex items-center justify-center gap-2">
                {cancellingTicket && <Loader2 size={18} className="animate-spin" />}キャンセルする
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl p-5 shadow-xl space-y-3 animate-fade-in">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => setConfirming(false)}
                className="py-4 rounded-xl bg-gray-100 text-gray-600 font-bold active:scale-95 transition-transform">いいえ</button>
              <button onClick={handleComplete} disabled={loading}
                ...
                {loading && <Loader2 size={18} className="animate-spin" />}はい
              </button>
            </div>
          </div>
        )}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-600 to-teal-700 flex flex-col items-center justify-center px-6">
      <div className="text-center animate-slide-up">
        ...
        <button onClick={onReset} className="bg-white text-emerald-700 text-xl font-black py-5 px-10 rounded-2xl shadow-xl active:scale-95 transition-transform">もう一度受付する</button>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="min-h-screen bg-zinc-900 flex flex-col items-center justify-center px-6">
      <div className="text-center animate-slide-up">
        ...
        <p className="text-zinc-400 text-lg mb-10">再度並ぶ場合は最後尾からとなります</p>
  ...
  return (
    <div className="min-h-screen bg-zinc-900 flex flex-col items-center justify-center px-6">
      <div className="text-center animate-slide-up">
        ...
        <p className="text-zinc-400 text-lg mb-10">ご不在のためキャンセルされました</p>
        <button onClick={onReset} className="bg-gradient-to-r from-indigo-600 to-violet-600 text-white text-xl font-black py-5 px-10 rounded-2xl shadow-xl active:scale-95 transition-transform">もう一度受付する</button>
      </div>
    </div>
  )
...
export default function QueuePage() {
    ...
    ;(async () => {
      ...
      if (profile?.userId) {
        supabase.from('customers').select('*')
          .eq('store_id', storeId).eq('line_user_id', profile.userId)
          .order('created_at', { ascending: true })
          .then(({ data }) => { if (data && data.length > 0) setExistingCustomers(data as Customer[]) })
        try {
          const res = await fetch(`/api/check-friend?userId=${profile.userId}`)
          const { friend } = await res.json()
  ...
  if (view === 'loading') return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: `linear-gradient(135deg, ${theme.colors.primary}, ${theme.colors.primaryDark})` }}>
      <div className="text-center text-white">
        <Loader2 size={48} className="animate-spin mx-auto mb-4 text-white/70" />
  ...
  if (view === 'details' && ticket) return (
    <DetailsView ticket={ticket} storeId={storeId} onComplete={handleDetailsComplete} onSkip={handleDetailsSkip} />
  )
