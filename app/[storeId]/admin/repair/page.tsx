...
import {
  ...
  CalendarDays, Pencil, ChevronDown, AlertCircle,
} from 'lucide-react'
    ...
    <button onClick={onClick}
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-zinc-500 truncate">{customer.tel ?? 'TEL未登録'}</p>
        </div>
...
// お直しアイテム
// ============================================================
function RepairItem({ repair, onComplete, onDeliver }: {
...
// 新規顧客フォーム
// ============================================================
function NewCustomerForm({ storeId, initialName, initialTel, onSaved, onCancel }: {
  storeId: string; initialName?: string; initialTel?: string
...
export default function RepairManagementPage() {
  ...
  // 検索フィルター（名前・フリガナ・電話番号）
  const filtered = searchQuery.trim()
    ? customers.filter(c =>
        c.name.includes(searchQuery) ||
        (c.kana  ?? '').includes(searchQuery) ||
        (c.tel   ?? '').replace(/-/g, '').includes(searchQuery.replace(/-/g, ''))
      )
    : customers
    ...
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-amber-950/40 border border-amber-500/20 rounded-2xl p-3.5 text-center">
            <p className="text-xs text-amber-400/70 font-bold">預かり中</p>
        ...
        {loading ? (
          ...
          <div className="text-center py-8 text-zinc-600">
            ...
            <p className="text-sm">{searchQuery ? '該当する顧客が見つかりません' : '顧客が登録されていません'}</p>
          </div>
        ) : (
          <div className="space-y-2">
            {filtered.map(c => (
