...
import {
  Settings, Loader2, Plus, Trash2, AlertCircle, Save,
  CalendarDays, Clock, CheckCheck, LayoutDashboard, ChevronRight, Users,
  PackageSearch, GraduationCap,
} from 'lucide-react'
...
export default function SettingsPage() {
  ...
  const fetchSettings = useCallback(async () => {
    ...
    const { data } = await (supabase as any)
      .from('stores')
      .select('name, notice_threshold, wait_thresholds, allow_remote, notification_plan, push_settings, alert_days_repair, alert_days_purchase, repair_notes, welcome_message, notice_text, business_hours, order_schedule')
      .eq('id', storeId)
      .single()
    if (data) {
      ...
      <div className="max-w-2xl mx-auto px-4 py-5 space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
            <div className="px-4 pt-3 pb-1">
              <p className="text-xs font-bold text-gray-600">営業時間</p>
              <p className="text-[10px] text-gray-400 mt-0.5">受付ページ・管理画面での参照用。自動開閉は受付管理で手動設定してください。</p>
            </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1.5">
            <p className="text-xs text-gray-500">待ち案内メッセージ（顧客画面表示）</p>
        ...
        {/* ③ 案件管理設定 */}
        <div className="space-y-3">
          <SectionLabel title="案件管理設定" />

          {/* 未お渡しアラート */}
          <div className="space-y-2">
            <p className="text-xs text-gray-500">未お渡しアラート（完了・入荷からN日後に警告）</p>
            {[
          ...
          {/* 発注促進スケジュール */}
          <div className="space-y-2">
            <p className="text-xs text-gray-500">発注促進スケジュール</p>
            <p className="text-[10px] text-gray-400">設定した条件を満たすと、管理画面上部に「発注タイミングです！」バナーが表示されます。</p>
            <Toggle
              on={orderScheduleOn}
              onToggle={() => {
          ...
          {/* お直し持込注意事項 */}
          <div className="space-y-1.5">
            <p className="text-xs text-gray-500">お直し持込 注意事項（顧客向けページ）</p>
            <p className="text-[10px] text-gray-400">リッチメニュー「依頼」から開くページに表示されます。空欄の場合はデフォルト案内文が表示されます。</p>
            <textarea
              value={repairNotes}
