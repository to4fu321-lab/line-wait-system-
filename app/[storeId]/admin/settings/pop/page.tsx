...
import { PopPreview } from '../../_components/PopPreview'
...
export default function PopEditorPage() {
  ...
  const load = useCallback(async () => {
    ...
    const { data: store } = await (supabase as any)
      .from('stores')
      .select('name, business_hours, allow_remote, notification_plan, pop_settings')
      .eq('id', storeId)
      .single()
    ...
    setSettings(mergePopSettings(store?.pop_settings ?? null, f))
    setLoading(false)
  }, [storeId])
  ...
  const regenerateMerits = () => {
    ...
    setSettings(s => s ? { ...s, merits: generateMerits(flags) } : s)
  }
  ...
  const handleSave = async () => {
    ...
    const { error } = await (supabase as any).from('stores').update({ pop_settings: settings }).eq('id', storeId)
    setSaving(false)
  ...
  const handlePng = async () => {
    ...
    try {
      ...
      a.href = dataUrl
      a.download = `${storeName || '店舗'}_友だち登録POP.png`
      a.click()
    } catch (e) {
      ...
      <div className="sticky top-0 z-40 bg-gray-50/90 backdrop-blur border-b border-gray-100" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-base font-bold text-gray-900">友だち登録POP</h1>
      ...
      <div className="max-w-5xl mx-auto px-4 py-4 grid md:grid-cols-2 gap-4 pb-16">
        ...
        <div className="md:order-1 space-y-3">
          ...
          <Card title="基本情報" emoji="📝">
            <Field label="LINE友だち追加URL">
              <input type="url" value={settings.friendUrl} onChange={e => up({ friendUrl: e.target.value })}
                placeholder="https://lin.ee/xxxxxxx"
                className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:border-indigo-500 focus:outline-none" />
              <p className="text-[11px] text-gray-400 mt-1">LINE公式アカウント管理画面の「友だち追加URL」を貼り付けてください。ここからQRを自動生成します。</p>
            </Field>
            <Field label="大見出し">
          ...
          {/* メリット */}
          <Card title="友だち登録でできること（機能連動）" emoji="✨">
