  ...
  return (
    <div className={`rounded-2xl border p-4 transition-all ${
      item.kind === 'repair'
        ...
        : 'bg-gradient-to-br from-teal-950/40 to-emerald-950/30 border-teal-500/20'
    }`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 flex-wrap mb-1.5">
            ...
            />
            {item.notified && (
              <span className="text-[10px] bg-emerald-900/50 text-emerald-400 border border-emerald-500/20 px-1.5 py-0.5 rounded-full">
                LINE通知済み
              </span>
            )}
            {isOverdue && (
          ...
          {item.sub_label && <p className="text-zinc-400 text-xs mt-0.5">{item.sub_label}</p>}
          {item.price != null && <p className="text-zinc-500 text-xs mt-0.5">¥{item.price.toLocaleString()}</p>}
          {item.slip_number && <p className="text-zinc-600 text-xs mt-0.5 font-mono">#{item.slip_number}</p>}

          {/* 日付 */}
          <div className="flex items-center gap-3 mt-1.5 text-zinc-600 text-[10px]">
        ...
        <div className="mt-3 pt-3 border-t border-white/10 space-y-1 animate-fade-in">
          {item.child && (
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-amber-300 text-xs font-bold">{item.child.name}</span>
            </div>
          )}
          {item.customer.tel && (
            <a href={`tel:${item.customer.tel}`}
              className="flex items-center gap-1.5 text-blue-400 text-xs font-bold">
        ...
        <div className="mt-3 bg-zinc-900 border border-indigo-500/30 rounded-2xl p-4 space-y-3 animate-fade-in">
          ...
          <button onClick={() => setPayAtDeliver(v => !v)}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border-2 transition-all ${
              payAtDeliver ? 'border-emerald-500 bg-emerald-500/10' : 'border-zinc-700 bg-zinc-800/50'
            }`}>
            <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${
              payAtDeliver ? 'border-emerald-500 bg-emerald-500' : 'border-zinc-600'
            }`}>
              {payAtDeliver && <CheckCheck size={11} className="text-white" />}
            </div>
            <div className="text-left">
              <p className={`text-sm font-bold ${payAtDeliver ? 'text-emerald-300' : 'text-zinc-400'}`}>
                代金を受け取った
              </p>
          ...
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => setConfirmOpen(false)}
              className="py-3 rounded-xl font-bold text-sm bg-zinc-700 text-zinc-300 active:scale-95 transition-all">
              キャンセル
            </button>
            <button
              onClick={async () => {
...
function CompletedCard({ item, onRevert, onPaymentToggle }: {
  ...
  return (
    <div className="rounded-2xl border bg-zinc-900/30 border-zinc-800/40 p-4">
      <div className="flex items-start gap-3">
        <div className="shrink-0 mt-0.5 w-8 h-8 rounded-xl flex items-center justify-center bg-zinc-800">
        ...
        <div className="flex-1 min-w-0">
          {item.customer && (
            <button onClick={() => setCustOpen(v => !v)}
              className="text-xs font-bold text-zinc-400 mb-1 flex items-center gap-1 w-full text-left active:opacity-70">
              <User size={10} />
              {item.customer.name}
              {item.child && <span className="text-zinc-500">（{item.child.name}）</span>}
              <ChevronDown size={10} className={`ml-auto shrink-0 transition-transform text-zinc-600 ${custOpen ? 'rotate-180' : ''}`} />
            </button>
          )}

          <div className="flex items-center gap-1.5 flex-wrap mb-1">
            <span className="text-xs font-bold px-2 py-0.5 rounded-full border bg-zinc-700/60 text-zinc-400 border-zinc-600/50">
              お渡し済み
            </span>
            <PaymentBadge
              status={item.payment_status}
          ...
          <p className="font-bold text-zinc-400 text-sm">{item.item_name}</p>
          {item.sub_label && <p className="text-zinc-600 text-xs mt-0.5">{item.sub_label}</p>}
          {item.price != null && <p className="text-zinc-600 text-xs mt-0.5">¥{item.price.toLocaleString()}</p>}

          <div className="flex items-center gap-3 mt-1.5 text-zinc-700 text-[10px]">
            <span className="flex items-center gap-1">
              <CalendarDays size={9} />受付 {fmtDate(item.received_date)}
            </span>
            {item.delivered_date && (
      ...
      {custOpen && item.customer?.tel && (
        <div className="mt-2 pt-2 border-t border-white/5 animate-fade-in">
          <a href={`tel:${item.customer.tel}`} className="flex items-center gap-1.5 text-blue-400 text-xs font-bold">
            <Phone size={11} />{item.customer.tel}
          </a>
        </div>
      )}
      ...
      {!confirmRevert ? (
        <button onClick={() => setConfirmRevert(true)}
          className="w-full mt-3 py-2 rounded-xl font-bold text-xs border border-zinc-700 text-zinc-600 hover:text-zinc-400 hover:border-zinc-600 transition-all flex items-center justify-center gap-1.5 active:scale-95">
          <RotateCcw size={11} />お渡しを取り消す
        </button>
      ) : (
        <div className="mt-3 bg-amber-950/40 border border-amber-500/20 rounded-xl p-3 space-y-2 animate-fade-in">
          <p className="text-xs text-center text-amber-300 font-bold">お渡しを取り消して前の状態に戻しますか？</p>
      ...
      <div className="sticky top-0 z-30 bg-zinc-950/90 backdrop-blur-xl border-b border-white/5 px-4 py-3">
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="font-black text-white text-base">お渡し管理</h1>
            ...
            <button key={t.id}
              ...
              {t.count !== null && t.count > 0 && (
                <span className={`text-xs px-1.5 py-0.5 rounded-full font-black ${
                  tab === t.id ? 'bg-indigo-500 text-white' : 'bg-zinc-700 text-zinc-400'
                }`}>{t.count}</span>
              )}
            </button>
          ))}
        ...
        {tab === 'waiting' && (
          loading ? (
            ...
            <div className="text-center py-16 text-zinc-600">
              ...
              <p className="text-xs mt-1 text-zinc-700">お直し完了・入荷済みの商品がここに表示されます</p>
            </div>
          ) : (
