...
// ── UniformOrderGuideModal ────────────────────────────────────
function UniformOrderGuideModal({ maker, entry, onClose, onComplete }: {
  maker: string
  entry: UniformMakerEntry
  onClose: () => void
  onComplete: (ids: string[]) => Promise<void>
}) {
  const checklistItems = useMemo(() => {
    type Row = { key: string; school: string; item: string; size: string | null; count: number }
    const map = new Map<string, Row>()
    for (const school of entry.schools) {
      for (const item of school.items) {
        for (const sz of item.sizes) {
          const key = `${school.school_name}|${item.item_name}|${sz.size ?? ''}`
          if (!map.has(key)) map.set(key, { key, school: school.school_name, item: item.item_name, size: sz.size, count: 0 })
          map.get(key)!.count += sz.count
        }
      }
    }
  ...
  async function handleComplete() {
    if (!matches) return
    setCompleting(true)
    await onComplete(entry.allOrders.map(o => o.id))
    setCompleting(false)
  }
      ...
      <div className="w-full max-w-lg bg-white rounded-t-3xl shadow-2xl flex flex-col"
        ...
        {step === 'list' ? (
          ...
          <div className="flex-1 px-5 py-6 space-y-4">
            <div className="bg-indigo-50 border border-indigo-200 rounded-2xl p-5 text-center">
              <p className="text-xs font-bold text-indigo-600 mb-1">システム集計（発注すべき合計）</p>
              <p className="text-5xl font-black text-indigo-700 tabular-nums">{totalCount}</p>
              <p className="text-sm font-bold text-indigo-500 mt-1">点</p>
            </div>
            <div>
              <label className="text-sm font-bold text-gray-700 block mb-2">外部サイトで入力した合計数量を入力</label>
              <input type="number" value={confirmInput} onChange={e => setConfirmInput(e.target.value)}
                autoFocus inputMode="numeric"
                className="w-full text-center text-4xl font-black border-2 border-gray-300 rounded-2xl py-5 focus:border-indigo-500 focus:outline-none tabular-nums"
                placeholder="0" />
            ...
            {confirmInput !== '' && !matches && (
              <div className="bg-red-50 border border-red-300 rounded-2xl px-4 py-3 flex items-center gap-2">
                <AlertCircle size={16} className="text-red-500 shrink-0" />
                <p className="text-sm font-bold text-red-700">
                  システム集計（{totalCount}点）と一致しません。発注画面を再確認してください。
                </p>
              </div>
            )}
            {matches && (
        ...
        <div className="px-5 py-4 border-t border-gray-100 shrink-0 space-y-2">
          {step === 'list' ? (
            ...
            <>
              <button onClick={handleComplete} disabled={completing || !matches}
                className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 text-white font-black rounded-2xl disabled:opacity-40 active:scale-[0.98] transition-all flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/25">
                {completing ? <Loader2 size={16} className="animate-spin" /> : <CheckCheck size={16} />}
                発注済みにする（{entry.allOrders.length}件）
              </button>
              <button onClick={() => setStep('list')}
                className="w-full py-2 text-xs font-bold text-gray-500 hover:text-gray-700 transition-colors">
                ← チェックリストに戻る
...
function UniformMakerCard({ maker, onGuide }: {
  maker: UniformMakerEntry
  onGuide: () => void
}) {
  const [open, setOpen] = useState(false)
    ...
    <div className="rounded-2xl border-2 border-indigo-200 bg-white overflow-hidden shadow-sm">
      <div className="flex items-center gap-3 px-4 py-3.5">
        <button className="flex-1 text-left min-w-0" onClick={() => setOpen(v => !v)}>
          ...
          <p className="text-[10px] text-gray-400 mt-0.5">
            {maker.schools.length}校 · {maker.allOrders.length}注文
          </p>
        </button>
        <button onClick={onGuide}
          className="shrink-0 flex items-center gap-1.5 px-3 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-black rounded-xl active:scale-95 transition-all shadow-md shadow-indigo-200">
          <ClipboardList size={13} />発注ガイド
        </button>
        <button onClick={() => setOpen(v => !v)} className="shrink-0 text-gray-400 p-1">
          {open ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
            ...
            <div key={school.school_name} className="px-4 py-3">
              ...
              {school.items.map(item => (
                <div key={item.item_name} className="ml-2 mb-3 last:mb-0">
                  <div className="flex items-center gap-2 mb-1.5">
                    <p className="text-sm font-bold text-gray-800">{item.item_name}</p>
                    <span className="text-[10px] text-gray-400">計{item.totalCount}点</span>
                  </div>
                  {item.sizes.map(sz => (
                    <div key={sz.size ?? '_none'} className="flex items-center gap-2 ml-3 py-1">
                      <span className="text-xs font-bold text-indigo-600 min-w-[64px] shrink-0">
                      ...
                      <span className="text-xl font-black text-indigo-600 tabular-nums shrink-0">×{sz.count}</span>
                      <p className="text-[10px] text-gray-400 truncate">
                        {sz.orders.map(o => o.child?.name ?? o.customer?.name ?? '?').join('、')}
                      </p>
                    </div>
...
export function UniformMakerOrderPanel({ orders, onRefresh, onToast }: {
  orders: UniformOrderRow[]
  onRefresh: () => void
  onToast: (t: 'ok' | 'err', m: string, undo?: () => Promise<void>) => void
}) {
  const [guideFor, setGuideFor] = useState<UniformMakerEntry | null>(null)
  const makers = useMemo(() => buildUniformMakerHierarchy(orders), [orders])
  ...
  async function completeOrdering(ids: string[]) {
    ...
    onToast('ok', `${ids.length}件を発注済みにしました`, async () => {
      await (supabase as any).from('uniform_orders')
        .update({ status: 'confirmed', updated_at: new Date().toISOString() }).in('id', ids)
      onRefresh()
    })
    setGuideFor(null)
  }
  ...
  if (orders.length === 0) return (
    <div className="text-center py-6 text-gray-400">
      <ShoppingBag size={24} className="mx-auto mb-2 opacity-30" />
      <p className="text-sm">未発注の制服注文はありません</p>
    </div>
  )
  ...
  return (
    <>
      <div className="space-y-3">
        {makers.map(m => (
          <UniformMakerCard key={m.maker} maker={m} onGuide={() => setGuideFor(m)} />
        ))}
      </div>
      {guideFor && (
        <UniformOrderGuideModal
